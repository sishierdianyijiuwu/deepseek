 ...
 *
 * `web` is a hardcoded alias for `--profile web`; `hosted` aliases
 * `--profile hosted`; `plugin` manages a profile's plugin dependencies by
 * forwarding to pnpm.
...
Examples:
  ...
  dsh --profile hosted                       boot the hosted Account profile (same as: dsh hosted)
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  const hosted = program.command('hosted').description('boot the hosted Account profile (alias of --profile hosted); the web app\'s own flags follow')
  hosted
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the hosted web app (see: dsh hosted --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed hosted-profile tree (with the user layer and any --patch) and exit')
    .option('--dump-default-config', 'print the hosted profile\'s bundle layers (no user layer) and exit')
    .action((args: string[], options: BootOptions) => {
      rejectParentOptions('hosted')
      resolved = resolveBoot(hosted, 'hosted', options, args)
    })
