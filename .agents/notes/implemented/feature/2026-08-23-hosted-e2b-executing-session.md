...
Host `session.prompt` / `subagent.prompt` acquire the Account lock on the family-root Session after prompt admission. A second family is refused with `executing-session-busy`. Extra tabs may history/mux/prompt the same family; a follow-up after idle copy-back keeps the sandbox when the family is running again. `startExecutingSession`/`stopExecutingSession` share one per-Account chain. HTTP tests fake the E2B SDK.
