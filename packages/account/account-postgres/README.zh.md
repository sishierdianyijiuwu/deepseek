...
Password 以 scrypt 哈希存储。验证令牌和 Sign-in session id 存储为原始密钥的 SHA-256。该提供方注入 `ctx.mailer`，并在插入成功后发送验证消息。若发送抛出，`register` 返回 `mail_failed`；`resendVerification` 仍是静默成功，以免 HTTP 路由枚举 Unverified Account。
