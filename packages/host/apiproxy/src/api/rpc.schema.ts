...
export const rpcErrorSchema: z.ZodType<RpcError> = z.discriminatedUnion('code', [
  ...
  z.object({ code: z.literal('workspace-required'), message: z.string(), details: z.object({}) }),
  z.object({ code: z.literal('workspace-limit'), message: z.string(), details: z.object({ max: z.number() }) }),
  z.object({ code: z.literal('workspace-quota-exceeded'), message: z.string(), details: z.object({ maxBytes: z.number() }) }),
  z.object({ code: z.literal('workspace-import-refused'), message: z.string(), details: z.object({ gitUrl: z.string() }) }),
  ...
  z.object({ code: z.literal('credential-missing'), message: z.string(), details: z.object({}) }),
  z.object({ code: z.literal('executing-session-busy'), message: z.string(), details: z.object({ sessionId: z.string() }) }),
  z.object({
    code: z.literal('e2b-cap-exhausted'),
    message: z.string(),
    details: z.object({ capMinutes: z.number(), resetsAt: z.number() }),
  }),
  z.object({ code: z.literal('operator-access-readonly'), message: z.string(), details: z.object({}) }),
