...
托管 profile 组合包：在 `@deepseek-ai/dsh-base` + `@deepseek-ai/dsh-web-app` 之上插入 Account 的 PostgreSQL 提供方、SMTP 邮件发送、鉴权 HTTP Consumer、注册／登录／密码重置 UI、按 Account 隔离的 Credential，以及云空 Workspace。本地 `dsh web` 仍是不含 Account 的独立 profile。
