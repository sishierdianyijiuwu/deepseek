...
Hosted-profile bundle: a patch layer over `@deepseek-ai/dsh-base` + `@deepseek-ai/dsh-web-app` that inserts the Account PostgreSQL provider, SMTP mailer, auth HTTP Consumer, and register/sign-in/password-reset UI, and replaces the process-wide credentials row with the Account-scoped provider. Local `dsh web` stays a separate profile without Accounts.
...
- **Does not isolate Workspaces** — a later ticket binds Workspace list/create/Import to the Sign-in session.
