...
| `deleteOwned(owner): Promise<void>` | Erase persisted Sessions whose header `owner` is this Account id. JSONL removes those session directories; SQLite deletes matching rows. Backends that do not store Account-owned logs no-op. Unknown owners are a no-op. |
