...
写入（`set` / `unset` / `modifyRecord` / `deleteRecord`）要求 Sign-in session 通过 `currentAccountId()` 绑定 Account。没有绑定 Account 的调用把该引用描述为未配置且不可写，`resolve` 返回 `undefined`，而不是另一 Account 的机密。
