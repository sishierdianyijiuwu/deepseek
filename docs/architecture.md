...
A **profile** is a named composition stored in the Harness home. It lists the bundles it stacks, holds any out-of-tree plugins it installs, and keeps the user's own `cordis.patch.yml`. `web`, `hosted`, and `headless` ship as templates.
...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-hosted`](../packages/bundle/hosted/README.md) layers Accounts over web-app and keeps dsh on loopback, with HTTPS at a reverse proxy ([cookbook](cookbook/hosted-tls-reverse-proxy.md)); [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
