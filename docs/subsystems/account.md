...
The hosted Account capability: `ctx.accounts` persists Account and Sign-in session rows; `ctx.mailer` delivers verification and password-reset messages. Product vocabulary is [CONTEXT.md](../../CONTEXT.md). HTTP routes live on [`dsh-account-http`](../../packages/account/account-http/README.md).
